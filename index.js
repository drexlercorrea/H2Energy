...
function receberDados() {
  ...
  .then(data => {
    ...
    function preencherTabela() {
      ...
      var dia = dataAtual.getUTCDate(); var mes = dataAtual.getUTCMonth() + 1; var ano = dataAtual.getUTCFullYear();
  ...
  .then(data => {
    ...
    for (let i = 0; i < dadosAirtable.length; i++) {
      ...
      if (nomeAirtable == nameTagUfv) {
        ...
        enviarProjeto(idOriginalAirtable); receberProjetos(idOriginalAirtable);
      }
    }
  })
  .catch(error => {
      ...
      try {
        ...
        .then(response => {
          if (response.ok) {
            ...
            document.getElementById("listaFolha").value = "";
            document.getElementById("revisao").value = "";
            abrirFormularioPrancha(); receberProjetos(idOriginalAirtable);
          }
        });
...
function receberProjetos(idOriginalAirtable) {
  fetch("https://api.airtable.com/v0/appJFXS6s2GGwHKyw/projetosexecutivos?filterByFormula=id%3D" + idOriginalAirtable + "&sortField=data",
  {
    method: "GET",
    headers: {
      Authorization: "Bearer keyggOsGLubPoGKDd",
